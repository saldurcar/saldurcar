"""Public portfolio LSTM example for battery thermal-stage classification."""
...
def build_lstm(n_steps, n_features, n_classes):
    model = Sequential(
        [
...
def train_from_dataframe(df, feature_columns, output_dir):
    ...
    model = build_lstm(n_steps, n_features, len(classes))
    callbacks = [
    ...
    model.save(output_dir / "lstm_stage_classifier.keras")
